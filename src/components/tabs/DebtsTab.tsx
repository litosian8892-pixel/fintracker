        ...
        <div className="space-y-5 animate-in slide-in-from-right duration-250 text-left pb-12">
          <div className="flex items-center justify-between bg-white dark:bg-slate-900 p-3.5 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm">
            ...
            <div className="flex gap-2">
              <button 
                ...
                className="p-2 bg-slate-50 hover:bg-slate-100 dark:bg-slate-800 dark:hover:bg-slate-700 rounded-xl transition-all text-blue-600 cursor-pointer border border-slate-200/40 dark:border-slate-700/40"
              >
                <Pencil size={15} />
              </button>
              <button 
          ...
          <div className="bg-white dark:bg-slate-900 p-6 rounded-[28px] border border-slate-100 dark:border-slate-800 shadow-sm space-y-6">
            <div className="flex justify-between items-start">
              ...
              <span className="px-2.5 py-1 bg-amber-50 dark:bg-amber-900/40 border border-amber-100/50 dark:border-amber-800/30 rounded-full text-[9px] font-black text-amber-600 dark:text-amber-400 tracking-wider">
            ...
            <div className={`p-4 rounded-2xl border flex items-center justify-between ${
              selectedDebt.type === "debt" 
                ? "bg-red-50/50 dark:bg-red-900/20 border-red-100/80 dark:border-red-800/30 text-red-700 dark:text-red-400" 
                : "bg-emerald-50/50 dark:bg-emerald-900/20 border-emerald-100/80 dark:border-emerald-800/30 text-emerald-700 dark:text-emerald-400"
            }`}>
          ...
          <div className="bg-white dark:bg-slate-900 p-5 rounded-[26px] border border-slate-100 dark:border-slate-800 shadow-sm space-y-4">
            <div className="flex justify-between items-center px-1">
              ...
              <span className="px-2 py-0.5 bg-blue-100/60 dark:bg-slate-800 text-[9px] font-black rounded-full text-blue-600 dark:text-blue-400 border border-transparent dark:border-slate-700">
            ...
            <div className="fixed inset-0 z-[180] flex items-end sm:items-center justify-center p-4 bg-slate-900/60 animate-in fade-in duration-200">
              ...
              <div className="bg-white dark:bg-slate-950 w-full max-w-md rounded-t-[32px] sm:rounded-[28px] shadow-2xl p-6 overflow-hidden z-10 flex flex-col max-h-[85vh] border border-slate-200 dark:border-slate-800 text-left animate-in slide-in-from-bottom sm:zoom-in-95 duration-250">
                <div className="flex justify-between items-center mb-4">
                  <h5 className="text-[10px] font-black text-blue-600 dark:text-blue-400 uppercase tracking-widest">Catat Cicilan / Pelunasan</h5>
                ...
                <div className="space-y-4">
                  <div className="space-y-1">
                    ...
                    />
                    {payAmount && <p className="text-[10px] font-bold text-slate-500 pl-1">Terbaca: <span className="font-black text-blue-600 dark:text-blue-400">{formatRupiahTerbaca(payAmount)}</span></p>}
                  </div>

                  <button 
                    type="button" 
                    onClick={() => { triggerHaptic(); setPayAmount((selectedDebt.amount - selectedDebt.paidAmount).toString()); }} 
                    className="w-full py-2.5 bg-blue-100 hover:bg-blue-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-[10px] font-black text-blue-700 dark:text-blue-400 rounded-lg border border-transparent cursor-pointer transition-all active:scale-95 text-center"
                  >
        ...
        <div className="space-y-6 animate-in fade-in duration-200">
            ...
            <button 
              ...
              className={`flex-1 py-3 text-xs font-black rounded-xl transition-all cursor-pointer flex items-center justify-center gap-2 ${
                mainTab === "subscriptions" 
                  ? "bg-blue-50/80 dark:bg-blue-900/40 text-blue-600 dark:text-blue-400 shadow-sm" 
              ...
              {!showAddSubForm ? (
                ...
                <div className="bg-slate-50 dark:bg-slate-950 p-5 rounded-[24px] border border-slate-200 dark:border-slate-800 space-y-3.5 text-left shadow-sm">
                  <div className="flex justify-between items-center px-1 mb-1">
                    ...
                    <button onClick={() => { setShowAddSubForm(false); setActiveKeypad(null); }} className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-300"><X size={15}/></button>
                  </div>
                  ...
                  <div className="space-y-1">
                    ...
                    {subAmount && (
                      <p className="text-[10px] font-bold text-slate-500 pl-1">
                        Terbaca: <span className="text-blue-600 dark:text-blue-400 font-black">{formatRupiahTerbaca(subAmount)}</span>
                      </p>
                    )}
                  </div>
                      ...
                      >
                        {editingSubId === sub.id ? (
                          <div className="space-y-3 pb-1">
                            ...
                            <input type="text" className="w-full p-3 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-xs font-bold text-slate-800 dark:text-white" value={editSubName} onChange={e => setEditSubName(e.target.value)} />
                            <input type="text" className="w-full p-3 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-xs font-bold text-slate-800 dark:text-white" value={editSubAmount} onChange={e => setEditSubAmount(e.target.value)} />
                            
                            <div className="grid grid-cols-2 gap-3">
                              <select className="w-full p-3 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-xs font-bold text-slate-800 dark:text-white" value={editSubCycle} onChange={e => setEditSubCycle(e.target.value as any)}>
                                <option value="monthly">Bulanan</option>
                                <option value="yearly">Tahunan</option>
                              </select>
                              <input type="date" className="w-full p-3 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-xs font-bold cursor-pointer text-slate-800 dark:text-white" value={editSubDueDate} onChange={e => setEditSubDueDate(e.target.value)} />
                            </div>

                            <select className="w-full p-3 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-xs font-bold cursor-pointer text-slate-800 dark:text-white" value={editSubAccountId} onChange={e => setEditSubAccountId(e.target.value)}>
                              <option value="" disabled>Pilih dompet...</option>
                              {accounts.map(acc => <option key={acc.id} value={acc.id}>{acc.name}</option>)}
                            </select>

                            <select className="w-full p-3 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-xs font-bold cursor-pointer text-slate-800 dark:text-white" value={editSubCategory} onChange={e => setEditSubCategory(e.target.value)}>
                              <option value="" disabled>Pilih kategori...</option>
                              {categories.filter(c => c.type === "expense").sort((a,b)=>a.name.localeCompare(b.name)).map(cat => <option key={cat.id} value={cat.name}>{cat.name}</option>)}
                            </select>

                            <div className="flex gap-2 pt-2">
                              <button onClick={() => submitEditSub(sub.id)} className="flex-1 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-bold shadow-sm cursor-pointer transition-all active:scale-95">Simpan Koreksi</button>
                              ...
                              <button 
                                ...
                                className={`flex-1 py-3 rounded-xl text-xs font-bold shadow-sm cursor-pointer flex items-center justify-center gap-1.5 transition-all active:scale-[0.98] ${
                                  isOverdue || isToday 
                                    ...
                                    : 'bg-blue-50/40 hover:bg-blue-100/60 dark:bg-blue-900/40 dark:hover:bg-blue-800/60 text-blue-600 dark:text-blue-400 border border-blue-100/30 dark:border-blue-800/30'
                                }`}
                              >
          ...
          {mainTab === "debts" && (
            <button 
              ...
              className="fixed bottom-24 right-6 md:bottom-8 md:right-8 z-50 p-4 bg-blue-600 hover:bg-blue-700 hover:scale-105 active:scale-95 text-white rounded-full shadow-2xl transition-all cursor-pointer border border-blue-500/10"
            >
        ...
        <>
          ...
          <div className="fixed bottom-0 left-0 right-0 z-[150] bg-white dark:bg-slate-950 border-t border-slate-200/80 dark:border-slate-800/80 p-4 pb-6 transition-all duration-300 md:max-w-md md:mx-auto md:rounded-t-[28px] md:shadow-2xl translate-y-0 text-slate-800 dark:text-white">
            <div className="flex justify-between items-center mb-3.5 px-1">
              ...
              <button onClick={() => setActiveKeypad(null)} className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 p-1 text-xs font-bold flex items-center gap-1.5">
                Selesai <X size={14} />
              </button>
            </div>
            ...
            <div className="grid grid-cols-4 gap-2 text-slate-800 dark:text-slate-100 font-black text-sm">
              ...
              {["(", "0", ")"].map((char) => (
                <button 
                  ...
                  className={`${char === "0" ? "bg-slate-50/90 dark:bg-slate-900/40 active:bg-slate-100 dark:active:bg-slate-800" : "bg-slate-100 dark:bg-slate-900 active:bg-slate-200 dark:active:bg-slate-800"} py-3.5 rounded-xl transition-all select-none border border-slate-200/30 dark:border-slate-800/10`}
                >
                  {char}
                </button>
              ))}
              <button 
                ...
                className="py-3.5 bg-blue-600 hover:bg-blue-500 active:bg-blue-700 rounded-xl text-white font-black shadow-md shadow-blue-900/20 transition-all select-none cursor-pointer"
              >
                Ya
              </button>
            </div>
        ...
        <div className="fixed inset-0 z-[190] flex items-end justify-center bg-slate-900/60 animate-in fade-in duration-200">
          ...
          <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-t-[32px] shadow-2xl p-6 pb-8 overflow-hidden z-10 flex flex-col max-h-[85vh] border-t border-slate-200 dark:border-slate-800 text-left">
            ...
            <div className="flex justify-between items-center mb-6 pt-2">
              ...
              <button type="button" onClick={() => setPayAccSelector(false)} className="p-1.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-500 rounded-full transition-colors cursor-pointer"><X size={14} className="text-slate-700 dark:text-slate-300" /></button>
            </div>
                  ...
                  return (
                    <div 
                      ...
                      className={`p-4 rounded-2xl border text-left flex flex-col justify-between relative transition-all active:scale-95 cursor-pointer h-28 ${
                        isSelected 
                          ? "border-blue-600 bg-blue-50/50 dark:bg-blue-900/20 shadow-md shadow-blue-500/5" 
