...
import { LogOut, Tag, CreditCard, X, Edit2, Check, Sun, Moon, Monitor, ChevronDown, ChevronUp, Trash2 } from "lucide-react";
...
export default function SettingsTab({
  ...
  return (
    <div className="space-y-6 animate-in fade-in">
      ...
      {/* KELOLA KATEGORI DENGAN DESAIN 2-KOLOM & 5-TERATAS */}
      <div className="bg-white dark:bg-slate-900 p-6 rounded-[30px] border border-slate-200 dark:border-slate-800 shadow-sm space-y-4 transition-colors duration-200">
        ...
        {/* LIST KATEGORI (SPLIT 2 KOLOM UNTUK PENGELUARAN) */}
        {tType === "expense" ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-4 pt-2 items-start">
            ...
            <div className="space-y-3">
              <div className="pb-2 border-b border-orange-100 dark:border-orange-900/30">
                <p className="text-[10px] font-black text-orange-500 uppercase tracking-widest flex items-center justify-between">
                  <span>🟠 Variabel (Sering)</span>
                  <span className="text-slate-400 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-full">{varCats.length}</span>
                </p>
              </div>
            ...
            {/* KOLOM KANAN: TETAP (FIXED) */}
            <div className="space-y-3">
              <div className="pb-2 border-b border-purple-100 dark:border-purple-900/30">
                <p className="text-[10px] font-black text-purple-500 uppercase tracking-widest flex items-center justify-between">
                  <span>🟣 Tetap (Bulanan)</span>
                  <span className="text-slate-400 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-full">{fixedCats.length}</span>
                </p>
              </div>
              <div className="space-y-2">
          ...
          /* TAMPILAN PEMASUKAN (1 KOLOM FULL) */
          <div className="pt-2 max-w-xl mx-auto w-full space-y-3">
             <div className="pb-2 border-b border-emerald-100 dark:border-emerald-900/30">
                <p className="text-[10px] font-black text-emerald-500 uppercase tracking-widest flex items-center justify-between">
                  <span>🟢 Daftar Pemasukan</span>
                  <span className="text-slate-400 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-full">{incomeCats.length}</span>
                </p>
              </div>
        ...
        <div className="flex flex-wrap gap-2 pt-2">
          {walletTypes.map(t => (
            <span key={t.id} className="bg-white dark:bg-slate-800 px-3 py-2 rounded-xl text-[10px] font-bold flex items-center gap-2 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 shadow-sm transition-colors duration-200">
              ...
              <X size={12} className="text-red-500 dark:text-red-400 cursor-pointer hover:scale-125 transition-transform" onClick={() => deleteWalletType(t.id)}/>
            </span>
          ))}
        </div>
      </div>
