...
export default function AssetsTab({
  ...
  const totalPersonal = personalActiveAccounts.reduce((sum, curr) => sum + (curr.balance * getRate(curr.currency, curr.lastExchangeRate, curr.id)), 0);
  ...
  const totalSeparated = separatedAccounts.reduce((sum, curr) => sum + (curr.balance * getRate(curr.currency, curr.lastExchangeRate, curr.id)), 0);
  ...
  const displayTotal = walletGroup === "pribadi" ? totalPersonal : walletGroup === "bisnis" ? totalBusiness : totalSeparated;
  const displayLabel = walletGroup === "pribadi" ? "Total Saldo Pribadi" : walletGroup === "bisnis" ? "Total Saldo Bisnis" : "Total Saldo Terpisah";
  const totalActiveBalance = displayTotal; 
  
  const globalActiveAccountIds = useMemo(() => new Set(personalActiveAccounts.concat(businessActiveAccounts).concat(separatedAccounts).map(a => a.id)), [personalActiveAccounts, businessActiveAccounts, separatedAccounts]);
  const dynamicAccountIds = useMemo(() => new Set(activeWalletsToRender.map(a => a.id)), [activeWalletsToRender]);
  ...
  const historicalAccountsData = useMemo(() => { let runningBalance = displayTotal; const data = []; const today = new Date(); for (let i = 0; i < 30; i++) { const d = new Date(today); d.setDate(d.getDate() - i); const dateStr = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`; const dayName = d.toLocaleDateString('id-ID', { day: 'numeric', month: 'short' }); data.unshift({ name: dayName, dateStr, Balance: runningBalance }); const dayTxs = reportTransactions.filter(t => t.tDate === dateStr); const dayInc = dayTxs.filter(t => t.type === 'income' && dynamicAccountIds.has(t.accountId)).reduce((sum, t) => sum + t.amount, 0); const dayExp = dayTxs.filter(t => t.type === 'expense' && dynamicAccountIds.has(t.accountId)).reduce((sum, t) => sum + t.amount, 0); const tToAst = dayTxs.filter(t => t.type === 'transfer' && dynamicAccountIds.has(t.accountId) && !dynamicAccountIds.has(t.toAccountId || "")).reduce((sum, t) => sum + t.amount, 0); const tFromAst = dayTxs.filter(t => t.type === 'transfer' && !dynamicAccountIds.has(t.accountId) && dynamicAccountIds.has(t.toAccountId || "")).reduce((sum, t) => sum + t.amount, 0); const adminFees = dayTxs.filter(t => t.type === 'transfer' && t.adminFee && dynamicAccountIds.has(t.accountId)).reduce((sum, t) => sum + t.adminFee!, 0); runningBalance = runningBalance - dayInc + dayExp + tToAst - tFromAst + adminFees; } return data; }, [displayTotal, dynamicAccountIds, reportTransactions]);
  const historicalAssetsData = useMemo(() => { let runningBalance = totalAssets; const data = []; const today = new Date(); for (let i = 0; i < 30; i++) { const d = new Date(today); d.setDate(d.getDate() - i); const year = d.getFullYear(); const month = String(d.getMonth() + 1).padStart(2, '0'); const day = String(d.getDate()).padStart(2, '0'); const dateStr = `${year}-${month}-${day}`; const dayName = d.toLocaleDateString('id-ID', { day: 'numeric', month: 'short' }); data.unshift({ name: dayName, dateStr, Balance: runningBalance }); const dayTxs = reportTransactions.filter(t => t.tDate === dateStr); const dayInc = dayTxs.filter(t => t.type === 'income' && assetAccountIds.has(t.accountId)).reduce((sum, t) => sum + t.amount, 0); const dayExp = dayTxs.filter(t => t.type === 'expense' && assetAccountIds.has(t.accountId)).reduce((sum, t) => sum + t.amount, 0); const transfersFromActive = dayTxs.filter(t => t.type === 'transfer' && globalActiveAccountIds.has(t.accountId) && assetAccountIds.has(t.toAccountId || "")).reduce((sum, t) => sum + t.amount, 0); const transfersToActive = dayTxs.filter(t => t.type === 'transfer' && assetAccountIds.has(t.accountId) && globalActiveAccountIds.has(t.toAccountId || "")).reduce((sum, t) => sum + t.amount, 0); runningBalance = runningBalance - dayInc + dayExp - transfersFromActive + transfersToActive; } return data; }, [totalAssets, globalActiveAccountIds, assetAccountIds, reportTransactions]);
        ...
        <>
          ...
          {activeSubTab === "akun" && (
            <div className="space-y-4 animate-in slide-in-from-right-4 duration-300">
              <div className="bg-white dark:bg-slate-900 p-6 rounded-[30px] border border-slate-200 dark:border-slate-800 shadow-sm transition-all duration-300">
                <h2 className="text-3xl font-black text-slate-800 dark:text-white text-center mb-1">{isPrivacyMode ? 'Rp •••••••' : `Rp ${displayTotal.toLocaleString('id-ID')}`}</h2>
                <p className="text-[10px] font-bold text-slate-500 text-center mb-6">{displayLabel} • {activeWalletsToRender.length} Dompet Likuid</p>
                {renderAreaChart(historicalAccountsData, walletGroup === "terpisah" ? "#64748b" : walletGroup === "bisnis" ? "#f59e0b" : "#3b82f6", "Balance")}
              </div>
