...
export default function ReportsTab({
  ...
  // STATE BARU: Untuk Show More 5 Teratas
  ...
  return (
    <div className="space-y-6 animate-in">
      <div className="bg-white dark:bg-slate-900 p-6 rounded-[30px] border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col gap-4 transition-colors duration-200">
        ...
        <button onClick={handleExportToExcel} className="w-full py-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded-2xl text-xs font-bold flex items-center justify-center gap-2 cursor-pointer transition-colors shadow-lg shadow-emerald-600/20 active:scale-95"><Download size={14}/> Export Laporan Excel</button>
      </div>
      ...
      <div className="space-y-4">
        ...
        {/* VARIABEL LIST - DENGAN LIMIT TOP 5 & TOMBOL SHOW MORE */}
        <div className="bg-white dark:bg-slate-900 p-6 rounded-[30px] border border-slate-200 dark:border-slate-800 shadow-sm space-y-3 transition-colors duration-200">
          ...
          {sortedVarKeys.length === 0 ? <p className="text-xs text-slate-400 dark:text-slate-500 italic">Kosong</p> : (
            <div className="space-y-2">
              ...
              {sortedVarKeys.length > 5 && (
                <button onClick={() => setShowAllVar(!showAllVar)} className="w-full text-[10px] font-bold text-blue-500 dark:text-blue-400 py-2 hover:bg-slate-50 dark:hover:bg-slate-800/50 rounded-xl transition-colors cursor-pointer border border-dashed border-slate-200 dark:border-slate-700">
                  {showAllVar ? "Sembunyikan" : `Tampilkan ${sortedVarKeys.length - 5} Lainnya ↓`}
                </button>
              )}
      ...
      {sortedPieData.length > 0 && (
        <div className="bg-white dark:bg-slate-900 p-6 rounded-[30px] border border-slate-200 dark:border-slate-800 shadow-sm transition-colors duration-200">
          ...
          {/* LEGEND PIE CHART DENGAN LIMIT TOP 5 & TOMBOL SHOW MORE */}
          <div className="mt-5 pt-4 border-t border-slate-100 dark:border-slate-800 space-y-2.5">
            {displayedPieData.map((data, idx) => {
              ...
              // Cari index asli untuk pewarnaan yang konsisten
              ...
              return (
                <div key={idx} className="flex justify-between items-center text-xs font-bold animate-in fade-in">
                  <div className="flex items-center gap-2.5">
                    <div className="w-3 h-3 rounded-full shrink-0" style={{ backgroundColor: COLORS[originalIndex % COLORS.length] }}></div>
            ...
            {sortedPieData.length > 5 && (
              <button onClick={() => setShowAllPie(!showAllPie)} className="w-full text-center text-[10px] font-bold text-blue-500 dark:text-blue-400 pt-2 pb-1 hover:text-blue-600 dark:hover:text-blue-300 transition-colors cursor-pointer">
                {showAllPie ? "↑ Sembunyikan" : `Tampilkan ${sortedPieData.length - 5} Lainnya ↓`}
              </button>
            )}
          </div>
