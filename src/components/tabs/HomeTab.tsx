...
interface HomeTabProps {
  ...
  tAdminFee: string; setTAdminFee: (val: string) => void;
...
export default function HomeTab({
  ...
  tAmount, setTAmount, tAdminFee, setTAdminFee, tNote, setTNote, categories, accounts, handleTransaction
}: HomeTabProps) {
      ...
      <div className="bg-white p-6 rounded-[30px] border border-slate-200 shadow-sm space-y-4 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          <div className="relative">
              ...
              <input type="date" className="w-full pl-10 pr-4 py-3 bg-slate-50 rounded-2xl text-xs font-bold border-none outline-none cursor-pointer text-slate-800" value={tDate} onChange={(e) => setTDate(e.target.value)} />
          </div>
        ...
        <input type="number" placeholder="Nominal Rp" className="w-full p-4 bg-slate-50 rounded-2xl text-sm font-bold border-none outline-none text-slate-800" value={tAmount} onChange={(e) => setTAmount(e.target.value)} />
        ...
        {tType === "transfer" && (
          <input 
            ...
            className="w-full p-4 bg-blue-50/50 rounded-2xl text-sm font-bold border-none outline-none text-blue-900 placeholder-blue-300 animate-in slide-in-from-top-2" 
            value={tAdminFee} 
            onChange={(e) => setTAdminFee(e.target.value)} 
          />
        )}

        <input type="text" placeholder={tType === "transfer" ? "Catatan Transfer" : "Catatan (Opsional)"} className="w-full p-4 bg-slate-50 rounded-2xl text-sm border-none outline-none text-slate-800" value={tNote} onChange={(e) => setTNote(e.target.value)} />
