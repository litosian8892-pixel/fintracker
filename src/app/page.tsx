...
import { LogOut, ArrowUpCircle, ArrowDownCircle, History, Trash2, Edit2, Check, X, Calendar, Tag, CreditCard, Smartphone, Banknote, Settings, Home, PieChart, ArrowRightLeft, HelpCircle, Upload, ArrowUp, ArrowDown, Wallet } from "lucide-react";
...
export default function FintrackerApp() {
  ...
  return (
    <main className="min-h-screen bg-slate-50 md:flex">
      ...
      {/* ==================== 1. SIDEBAR: HANYA MUNCUL DI PC/DESKTOP (md:flex) ==================== */}
      <aside className="hidden md:flex flex-col w-64 bg-white border-r border-slate-200 fixed h-full z-30 justify-between p-6 shadow-sm">
        <div className="space-y-8">
          ...
          <nav className="flex flex-col gap-2">
            <button onClick={() => setActiveTab("home")} className={`flex items-center gap-3 p-3.5 rounded-2xl text-xs font-black tracking-wide transition-all ${activeTab === "home" ? "bg-blue-50 text-blue-600 shadow-sm" : "text-slate-400 hover:bg-slate-50"}`}>
              <Home size={18} />
              <span>Beranda</span>
            </button>
            <button onClick={() => setActiveTab("reports")} className={`flex items-center gap-3 p-3.5 rounded-2xl text-xs font-black tracking-wide transition-all ${activeTab === "reports" ? "bg-blue-50 text-blue-600 shadow-sm" : "text-slate-400 hover:bg-slate-50"}`}>
              <PieChart size={18} />
              <span>Laporan</span>
            </button>
            <button onClick={() => setActiveTab("assets")} className={`flex items-center gap-3 p-3.5 rounded-2xl text-xs font-black tracking-wide transition-all ${activeTab === "assets" ? "bg-blue-50 text-blue-600 shadow-sm" : "text-slate-400 hover:bg-slate-50"}`}>
              <Wallet size={18} />
              <span>Aset & Akun</span>
            </button>
            <button onClick={() => setActiveTab("settings")} className={`flex items-center gap-3 p-3.5 rounded-2xl text-xs font-black tracking-wide transition-all ${activeTab === "settings" ? "bg-blue-50 text-blue-600 shadow-sm" : "text-slate-400 hover:bg-slate-50"}`}>
              <Settings size={18} />
              <span>Setting</span>
            </button>
          </nav>
        ...
        <div className="border-t border-slate-100 pt-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <img src={user?.photoURL || ""} className="w-8 h-8 rounded-full border border-blue-500 shadow-sm" />
            <p className="text-xs font-black text-slate-800 truncate max-w-[110px]">{user?.displayName?.split(" ")[0]}</p>
          </div>
          <button onClick={() => signOut(auth)} className="text-slate-400 hover:text-red-500 transition-colors p-1.5 rounded-lg hover:bg-red-50">
            <LogOut size={16} />
          </button>
        </div>
      </aside>

      {/* ==================== 2. AREA KONTEN UTAMA RESPONSIVE ==================== */}
      <div className="flex-1 md:ml-64 min-h-screen flex flex-col pb-24 md:pb-8">
        
        {/* HEADER ATAS: HANYA MUNCUL DI HP (md:hidden) */}
        ...
        {/* MASTER CONTAINER KONTEN (PINTAR & RESPONSIVE) */}
        <div className="max-w-5xl w-full mx-auto p-4 md:p-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            
            {/* ==================== KOLOM KIRI (LEBAR): KONTEN TAB AKTIF ==================== */}
                ...
                <div className="space-y-6 animate-in fade-in">
                  ...
                  <div className="bg-white p-6 rounded-[30px] border border-slate-200 shadow-sm space-y-4">
                    <div className="flex gap-2">
                      ...
                      <button onClick={() => setTType("transfer")} className={`flex-1 py-3 rounded-xl text-[10px] font-bold ${tType === "transfer" ? "bg-blue-50 text-white shadow-lg" : "bg-slate-100"}`}>TRANSFER</button>
                    </div>
                    ...
                    <select value={tAccountId} onChange={(e) => setTAccountId(e.target.value)} className="w-full p-4 bg-slate-50 rounded-2xl text-sm font-bold border-none outline-blue-500">
                    ...
                    {tType === "transfer" && (
                      <select value={tToAccountId} onChange={(e) => setTToAccountId(e.target.value)} className="w-full p-4 bg-blue-50/50 rounded-2xl text-sm font-bold border-none outline-blue-500">
              ...
              {activeTab === "reports" && (
                <div className="space-y-6 animate-in fade-in">
                  <div className="bg-white p-6 rounded-[30px] border border-slate-200 shadow-sm flex flex-col gap-4">
                    ...
                    <button onClick={handleExportToExcel} className="w-full py-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded-2xl text-xs font-bold flex items-center justify-center gap-2 shadow-lg active:scale-95 transition-all"><Download size={14}/> Export Bulan Ini ke Excel</button>
                  </div>
                            ...
                            <div className="flex justify-between items-center">
                              <div className="flex items-center gap-3">
                                {acc.logo ? (
                                  ...
                                  <div className="w-8 h-8 bg-slate-100 rounded-lg flex items-center justify-center text-slate-500">{getCardDesign(acc.type).icon}</div>
                                )}
                                <div>
            ...
            {activeTab === "settings" && (
              <div className="space-y-6 animate-in fade-in">
                <div className="bg-white p-6 rounded-[30px] border border-slate-200 shadow-sm flex flex-col items-center text-center space-y-4">
                  ...
                  <button onClick={() => signOut(auth)} className="px-6 py-2.5 bg-red-50 hover:bg-red-100 text-red-600 rounded-xl text-xs font-bold flex items-center gap-2 transition-all"><LogOut size={14}/> Logout dari Aplikasi</button>
                </div>
